...
} from "../../../../utils/metricResultsToText";
...
function CopyResultsButton({ metricGroups }: CopyResultsProps) {
    ...
    const [btnText, setBtnText] = React.useState(
        METRICS_TABLE_TEXT.COPY_RESULTS_BTN
    );
    ...
    function onCopySuccess() {
        ...
        displayNotification({
            content: "Results copied To clipboard successfully!",
            title: "Copy Results",
            duration: 2000,
            type: "success",
        });
        setTimeout(() => {
            setBtnText(METRICS_TABLE_TEXT.COPY_RESULTS_BTN);
        }, 2000);
    }
    function onCopyFailure() {
        setBtnText(METRICS_TABLE_TEXT.COPY_RESULTS_BTN_ERROR);
        displayNotification({
            content: "Results could not be copied to clipboard.",
            title: "Copy Results",
            duration: 2000,
            type: "error",
        });
    ...
    return (
        <button
            ...
            onClick={() => onClickBtn()}
            disabled={btnText !== METRICS_TABLE_TEXT.COPY_RESULTS_BTN}
        >
            {btnText}
        </button>
    );
