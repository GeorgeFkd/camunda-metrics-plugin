...
export default function StatsTableTitle() {
    ...
    //If there is no participant selected, then the
    //whole model text is being displayed
    if (currentParticipant.name === "") {
        msgDisplayed = STATS_TABLE_TEXT.WHOLE_MODEL;
        styleForNoParticipant = styles.noParticipant;
    } else {
        msgDisplayed =
            STATS_TABLE_TEXT.WITH_PARTICIPANT + currentParticipant.name;
    }
    return (
        <div className={styles.statsTableTitleContainer}>
            <span className={`${styles.statsTableTitle}`}>
                {STATS_TABLE_TEXT.TITLE}
            </span>
            <span
                className={`${styles.currentParticipant} ${styleForNoParticipant}`}
            >
                {msgDisplayed}
            </span>
        </div>
